...
function createPeerConnections(stream) {
  ...
  myPeerConnection = new PeerConnection(null,{optional: [{RtpDataChannels: true}]});
...
function gotRemoteStream(event){ //Success! Show the remote video...
  ...
  var channel = remotePeerConnection.createDataChannel("Mydata");
  channel.onopen = function(event) {channel.send('***sending a message');}
